(ns tic-tac-toe.records
  (:require [clojure.string :as str]
            ...
            #?(:clj [tic-tac-toe.records-jdbc :as jdbc])))
...
(defn valid-sql-replay [game-id]
  (let [game-data  (jdbc/->sql-game-data game-id)
        moves-data (jdbc/->sql-moves-data game-id)]
...
(defn save-game-postgres [state move]
  (let [{:keys [game-id turn-count current-token]} (->state-to-record state)]
    ...
    (jdbc/save-move-postgres game-id turn-count current-token move)))
...
(comment
  ...
  (defprotocol Records
    (save-game [this state move])
    (replay-game [this game-id])
    (last-game [this])
    )

  (defmulti save-game [spec state move])
  (defmulti replay-game [spec game-id])
  (defmulti last-game [spec])

  (defmethod save-game :sql)
  (defmethod save-game :edn)
  (defmethod save-game :memory)
  (defmethod save-game :noop)
