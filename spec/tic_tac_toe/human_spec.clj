  ...
  (context "valid input"
    ...
    (it "returns true when grid point is empty"
      (let [test-board [[\space :X :O]
                        [:X \space \space]
                        [:O :X \space]]]
        (should= true (board-3x3/space-available? test-board [0 0]))
        (should= true (board-3x3/space-available? test-board [2 2]))
        (should= true (board-3x3/space-available? test-board [1 1]))
        (should= true (board-3x3/space-available? test-board [1 2]))))

    (it "returns false when grid point is taken"
      (let [test-board [[\space :X :O]
                        [:X \space \space]
                        [:O :X \space]]]
        (should= false (board-3x3/space-available? test-board [0 1]))
        (should= false (board-3x3/space-available? test-board [0 2]))
        (should= false (board-3x3/space-available? test-board [1 0]))
        (should= false (board-3x3/space-available? test-board [2 0]))
        (should= false (board-3x3/space-available? test-board [2 1]))))
    )
  )